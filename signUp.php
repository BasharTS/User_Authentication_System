...
<head>
	...
	<script>
		function redirectToProcess(buttonText) {
	      ...
	      window.location.href = 'signUp.php?hashedButtonText=' + encodedHashedButtonText;
	    }
  </script>
							...
							<p>Click on word that makes up part of your favorite Idiom</p>
							 <?php
								...
								$startTime = microtime(true);
      							$_SESSION['result']['startTime'] = $startTime;
								...
								if(isset($_GET['hashedButtonText'])){
								    $pass1 = $_GET['hashedButtonText'];
								    $md5ed = array();
								    for ($i=0; $i < count($filtered_arr) ; $i++) {
								    	array_push($md5ed, md5($filtered_arr[$i]));
								    }
								    
								    if (in_array($pass1, $md5ed)) {
								    	$_SESSION['result']['pass1'] = $pass1;
										header("location: http://localhost/Graphical_System/signUp2.php");
								    }else{
