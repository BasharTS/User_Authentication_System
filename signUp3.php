...
<head>
	...
	<script>
		function redirectToProcess(buttonText) {
	      ...
	      window.location.href = 'signUp3.php?hashedButtonText=' + encodedHashedButtonText;
	    }
  </script>
				...
				<div class="container">
					<div class="agile_info">
						...
						<div class="login_info">
							<h2>Select Your Password --> STEP 3</h2>
							...
							<div id="result"> </div>
							 <?php
								...
								if(isset($_GET['hashedButtonText'])){
								    $pass3 = $_GET['hashedButtonText'];
								    $md5ed = array();
								    for ($i=0; $i < count($filtered_arr) ; $i++) {
								    	array_push($md5ed, md5($filtered_arr[$i]));
								    }

								    if (in_array($pass3, $md5ed)) {
								    	$_SESSION['result']['pass3'] = $pass3;
										header("location: http://localhost/Graphical_System/signUpImg.php");
								    }else{
								...
								if ($idioms_arr['feedback'] = 1){
									...
									for($i = 0; $i < $array_length; $i++){
										if ($i < 12) {
										 	...
										 	$word = $selected[$i];
										    echo "
										    		<button class='btn word' name='submit' style='background: maroon' id='myBtn' onclick='redirectToProcess(this.innerText)' >".$word."
										    		</button>";
										}
									}
								}
